// Health.cs — Updated: isEliminated flag agar non-peserta Sudden Death
//             tidak bisa trigger ReportPlayerDeath secara tidak sengaja.
// File ini menggantikan: Health/Health.cs
...
public class Health : MonoBehaviour
{
    ...
    private bool isEliminated = false; // true = diset oleh SetHealthToZero (bukan peserta SD)
    private int  lives       = 3;

    AudioManager audioManager;
    public GameManager gameManager;
    [SerializeField] private HealthP2 otherPlayerHealth;
    ...
    private void Awake()
    {
        ...
        if (otherPlayerHealth == null) otherPlayerHealth = FindObjectOfType<HealthP2>();

        if (GameData.Instance != null && GameData.Instance.player1Character != null)
            spriteRenderer.sprite = GameData.Instance.player1Character.aliveSprite;
    }
    ...
    public void TakeDamage(float _damage)
    {
        ...
        if (currentHealth <= 0 && !isDead)
        {
            ...
            {
                DisablePlayer(true);
                gameManager?.ReportPlayerDeath(1);
            }
            else
            {
                DisablePlayer(true);
    ...
    public void DisablePlayer(bool showDeathSprite)
    {
        ...
        if (showDeathSprite && spriteRenderer != null &&
            GameData.Instance?.player1Character != null)
            spriteRenderer.sprite = GameData.Instance.player1Character.deathSprite;
    ...
    public void EnablePlayer()
    {
        ...
        if (spriteRenderer != null && GameData.Instance?.player1Character != null)
            spriteRenderer.sprite = GameData.Instance.player1Character.aliveSprite;
    ...
    // Dipanggil GameManager untuk non-peserta Sudden Death.
    // Set health & lives ke 0, TIDAK trigger ReportPlayerDeath.
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        if (collision.gameObject.CompareTag("BulletP2") ||
            ...
            collision.gameObject.CompareTag("BulletP4"))
        {
